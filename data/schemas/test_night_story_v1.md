...
- `messages`：短信对象，字段为 `id`、`sender`、`body`、0～59 的整数 `unlock_minute`、`unlocks_broadcast_ids`、`statement_ids` 与 `fact_ids`。
- `broadcasts`：精确 3 项；字段为 `id`、`source`、`body`、`unlock_message_ids`、`unlock_event_ids`、`sets_condition_id`、`exclusive_group_id` 与 `fact_ids`。后两个字段可为空字符串；非空值必须是已声明的稳定 ID。
...
- `StoryEngine` 是 `statement_revealed`、`fact_confirmed`、广播条件和 02:00 收束的唯一权威。漏接只会生成 `PhoneSystem` 的真实来电记录，绝不揭示电话陈述。
