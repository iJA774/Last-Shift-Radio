...
func _run() -> void:
	...
	await create_timer(2.70, true, false, true).timeout
	_assert_true(loading.modulate.a >= 0.98, "完整可见的 3 秒停留结束前不得开始渐出。")
	_assert_true(not _has_finished, "3 秒完整停留结束前不得发出完成信号。")
	...
	_assert_true(elapsed_seconds >= 3.9 and elapsed_seconds <= 4.6, "真实加载过渡总时长应约为 4 秒，实际 %.3f 秒。" % elapsed_seconds)
...
func _finish() -> void:
	if _failure_count == 0:
		print("[测试][LoadingScreenTransition] 通过：0.5 秒渐入、3 秒完整停留与 0.5 秒渐出使用真实 Tween 时序。")
		quit(0)
		return
