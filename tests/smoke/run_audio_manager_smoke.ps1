...
$smokeAppData = [System.IO.Path]::GetFullPath((Join-Path -Path $temporaryBase -ChildPath 'last_shift_radio_audio_manager_smoke'))
...
try {
	...
	& $godotConsole --headless --path $projectRoot --script res://tests/smoke/test_audio_manager.gd
	if ($LASTEXITCODE -ne 0) {
		throw "AudioManager 专项测试失败，退出码：$LASTEXITCODE"
	}
}
finally {
