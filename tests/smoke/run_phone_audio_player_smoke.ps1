...
$smokeAppData = [System.IO.Path]::GetFullPath((Join-Path -Path $temporaryBase -ChildPath 'last_shift_radio_phone_audio_smoke'))
...
try {
	...
	& $godotConsole --headless --path $projectRoot --script res://tests/smoke/test_phone_audio_player.gd
	if ($LASTEXITCODE -ne 0) {
		throw "PhoneAudioPlayer 专项测试失败，退出码：$LASTEXITCODE"
	}
}
finally {
