...
## 与“阅读”作为意图交给 GameScreen。玩家公告入口属于中央麦克风，不在电脑上重复提供。
...
func bind_story_engine(story_engine: RefCounted) -> Dictionary:
	...
	for signal_name: StringName in [&"computer_entries_changed"]:
...
func show_unauthorized_broadcast(record: Dictionary) -> Dictionary:
	...
	if not bool(validation["ok"]):
		return validation
	_ending_record = record.duplicate(true)
...
func select_category(category: String) -> Dictionary:
	...
	if _is_ending_locked:
		return {"ok": false, "message": "02:00 强制收束中，电脑记录已锁定。"}
	_active_category = category
	_refresh()
...
func _refresh() -> void:
	...
	_refresh_tab_labels()
	_refresh_summary()
	_page_title_label.text = "夜间记录" if _is_ending_locked else String(CATEGORY_TITLES.get(_active_category, "电脑终端"))
	_refresh_notice()
	_clear_children(_content_box)
	if _is_ending_locked:
		_refresh_ending_record()
	else:
...
func _refresh_tab_labels() -> void:
	for category: String in PAGE_CATEGORIES:
		...
		button.disabled = _is_ending_locked
		...
		if button.disabled:
			button.tooltip_text = "不可用：02:00 强制收束中，电脑记录已锁定。"
		else:
...
func _refresh_summary() -> void:
	...
	if _is_ending_locked:
		_summary_label.text = "02:00 收束锁定　｜　记录不可修改"
		return
...
func _refresh_ending_record() -> void:
	if _ending_record.is_empty():
		_content_box.add_child(_make_text_card("夜班结束记录缺失，无法显示。"))
		return
	var body: String = String(_ending_record.get("body", ""))
	var source: String = String(_ending_record.get("source", "Studio A"))
	var tick: Variant = _ending_record.get("sent_at_tick", _ending_record.get("time_tick", -1))
	if body.strip_edges().is_empty() or typeof(tick) != TYPE_INT:
		_content_box.add_child(_make_text_card("夜班结束记录损坏，无法显示。"))
		return
	_content_box.add_child(_make_section_title("夜班结束记录"))
	_content_box.add_child(_make_text_card("时间：%s　｜　来源：%s\n%s" % [_format_game_time(int(tick)), source, body]))
